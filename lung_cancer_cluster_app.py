...
def train_model(df):
    ...
    # 군집 의미 매핑: 흡연량 중심값 기준으로 정렬
    centers = km.cluster_centers_
    smoke_idx = 1   # 흡연량 열 인덱스 (scaled 기준)
    order = np.argsort(centers[:, smoke_idx])  # 흡연↑ 순
    # order[0]=매우건강, order[1]=건강, order[2]=위험
...
def plot_clusters(df, labels, patient_smoke, patient_drink, patient_cluster):
    COLORS = {0: "#26a69a", 1: "#fdd835", 2: "#4a148c"}  # 청록, 노랑, 보라
